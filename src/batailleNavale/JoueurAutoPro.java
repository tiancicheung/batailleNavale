...
public abstract class JoueurAutoPro extends JoueurAvecGrille {
...
protected int etatdernier;
    ...
    public JoueurAutoPro(GrilleNavale g, String nom) {
        super(g, nom);
    ...
    public JoueurAutoPro(GrilleNavale g) {
        super(g);
        TailleGrille = g.getTaille();
        nbTirs = 0;
        dejaTirs = new Coordonnee[g.getTaille() * g.getTaille()];
...
private Coordonnee ModeChasse() {
        int taille = cibleNavireIllusion.tailleNavire();
        if (taille == 1) {
        //进入四周围绕攻击模式
